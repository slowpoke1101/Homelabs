...
STATE SO FAR: Basic router hardening. Router configured with 2 bridged networks; trusted and guest. guest lan devices are unable to communicate with eachother, the router, and the trusted net. pi4 running omv with plugins and gitea, has iptables rules allowing only trusted ip's to access file shares and gitea. ssh disabled but can be manually enabled when i need to administer from my laptop. 3-2-1 manual backups weekly and monthly.